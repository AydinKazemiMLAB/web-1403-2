...
// todo app: -------
use('POST', 'item', function (request, response) {
    readFile('./todo.json', 'utf8', function (error, fileData){
        if(error){
            console.log('ERROR:', error);
            write(response, 500, 'ERROR:' + error);
        }
            ...
            dataObject.records.push({
                text: request.data.text,
                status: false,
                id: id
            });
            let dataString = JSON.stringify(dataObject);
            writeFile('./todo.json', dataString, function (error){
                if(error){
                    console.log('ERROR:', error);
                    write(response, 500, 'ERROR:' + error)
                }
                    ...
                    write(response, 200, JSON.stringify({status:'Item Created.', id: id}))
                }
            });
        }
    });
});
use('GET', 'item', function (request, response) {
    readFile('./todo.json', 'utf8', function (error, fileData){
        if(error){
            console.log('ERROR:', error);
            write(response, 500, 'ERROR:' + error);
...
use("PUT", "item", function (request, response){
    readFile('./todo.json', 'utf8', function (error, fileData){
        if(error){
            console.log('ERROR:', error);
            write(response, 500, 'ERROR:' + error);
            ...
            if(!findIndex){
                write(response, 400, JSON.stringify({status: "Item does not exist", id: request.data.id}))
            }
            let dataString = JSON.stringify(dataObject);
            writeFile('./todo.json', dataString, function (error){
                if(error){
                    ...
                    console.log('Item Created.');
                    write(response, 200, JSON.stringify({status:'Item updated successfully', id: request.data.id}))
                }
            });
        }
    });
